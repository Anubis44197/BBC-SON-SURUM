...
from bbc_core.bbc_scalar import (
    BBCScalar, OmegaOperator, BBCEncoder, bbc_hook, bbc_data_ingestion,
    STABLE, WEAK, SLEEPING, NEG_ZERO, SATURATED, UNSTABLE, DEGENERATE
)
...
class HMPU_Governor:
    ...
    """
    def __init__(self, weights_path: str = "hmpu_weights.json", state_manager=None, heal_budget: int = 5, session_heal_budget: int = 5):
        ...
        self._iterations = 5
        # heal_limit yerine StateManager'ın session_heal_budget'i kullanılacak
    ...
    def aura_field_score(self, s: float, c: float, p: float) -> float:
        ...
        if degenerate_inputs:
            ...
            # Recovery attempted through StateManager, but we still signal the error
            ...
            for i in range(3):
                for j in range(3):
                    ...
                    if w.state in [NEG_ZERO, UNSTABLE, DEGENERATE]:
                        # Check global session budget first
