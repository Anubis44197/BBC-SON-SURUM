...
class BBCCLI:
    ...
    async def run_analysis(self, target_path, output_file, silent: bool = False):
        ...
        # Read actual file contents for token counting (skip in silent mode)
        raw_content = ""
        context_content = ""
        
        if not silent:
            # Collect all raw file contents
            try:
...
def audit_bbc_traces(project_path: str) -> Dict[str, Any]:
    ...
    check_paths = {
        ...
        "ai-context.json": os.path.join(project_root, "ai-context.json"),
        ...
        "BBC_INSTRUCTIONS.md": os.path.join(project_root, "BBC_INSTRUCTIONS.md"),
        "BBC_CONTEXT.md": os.path.join(project_root, "BBC_CONTEXT.md"),
...
def main():
    ...
    if args.command == "analyze":
        ...
        for name, meta in report.get("paths", {}).items():
            ...
            print(f"[{status:7}] {name} -> {meta.get('path')}")
        print(f"{'='*70}\n")
