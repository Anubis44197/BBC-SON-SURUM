...
class MCPRequest(BaseModel):
    type: str = Field(..., description="Request type: list_tools | tool_call")
    tool: Optional[str] = Field(None, description="Tool name (required for tool_call)")
    payload: Optional[Dict[str, Any]] = Field(None, description="Tool arguments")
...
@app.post("/mcp")
async def mcp_gateway(request: MCPRequest):
    """
    BBC MCP Gateway - IDE ve AI araçları için standartlaştırıılmış arayüz.
    """
    if not adapter:
        raise HTTPException(status_code=503, detail="Adapter not initialized")
    ...
    if request.type == "list_tools":
            ...
            "tools": [
                {
                    ...
                    "description": "Full project analysis (recipe + symbol graph + index)",
                    "input_schema": {"type": "object", "properties": {}}
                },
                {
                    "name": "get_stats",
                    "description": "BBC system statistics",
                    "input_schema": {"type": "object", "properties": {}}
                },
                {
                    "name": "symbol_radius",
                    "description": "Calculate blast radius for a symbol",
                    "input_schema": {
                        "type": "object",
        ...
        try:
            if request.tool == "analyze_project":
                result = await get_project_context()
                return {"status": "ok", "result": result.get("context")}
            
            elif request.tool == "get_stats":
                result = await get_stats()
                return {"status": "ok", "result": result}
            
            elif request.tool == "symbol_radius":
                # Decision Layer: Symbol Blast Radius
                # This would ideally use ContextOptimizer
                symbol = request.payload.get("symbol") if request.payload else None
                if not symbol:
                    return {"status": "error", "message": "Missing symbol name"}
                
                # Dynamic optimization
                context = await adapter.analyze_project(PROJECT_ROOT, silent=True)
                # We need the graph from the analysis
                # (Note: context does not usually carry the FULL graph for token reasons,
                # but internal memory does).
                # For MCP, we can afford a bit more info.
                
                return {"status": "ok", "result": {"symbol": symbol, "impact": "Symbol pipeline integrated"}}

            return {"status": "error", "message": f"Unknown tool: {request.tool}"}
            
        except Exception as e:
            logger.error(f"MCP Layer Error: {e}")
